...
int sorthelper(const void* e1, const void* e2) 
 { 
   char c1 = *(char*)e1; 
   char c2 = *(char*)e2; 
   return c1 - c2; 
...
int isValidState(const char * state)
{
  int count;
  char *copy;
  copy = malloc(strlen(state)+1);

  for(count = 0; count<=strlen(state);count ++)
    {
      copy[count] = state[count];
    }
  qsort(copy, strlen(state),sizeof(char),sorthelper);
...
{
  int count;
  for(count = 0; count < strlen(moves); count ++)
    {
...
{    
  ...
  //1a 
  while(state[pos] != '-' && pos < strlen(state))
    {
      pos++;
    }
  //1b
  ...
  newpos = (newrow * SIDELENGTH) + newcol;
  swaper = state[pos];
...
void processMoveList(char * state, const char * movelist)
{
  ...
  //int validstate;
  //1
  for(count = 0; count < strlen(movelist) && test == TRUE; count++)
    {
      test = move(state, movelist[count]);
    }
  if(test == FALSE)
    {
      printf("I\n");
...
MoveTree * MoveTree_create(const char * state, const char * moves)
{
  ...
  tree-> state = malloc((strlen(state)+1)*sizeof(char));
  tree-> moves = malloc((strlen(moves)+1)*sizeof(char));
  for(count = 0; count <= strlen(state); count ++)
    {
      tree->state[count] = state[count];
    }
  for(count = 0; count <= strlen(moves); count ++)
    {
      tree->moves[count] = moves[count];
    }
  tree -> left = NULL;
  tree -> right = NULL;
			   ...
			   const char * moves)
{
  ...
  if(strcmp(state, node -> state) == 0)
	...
	{
	  free(node->moves);
	  node -> moves = malloc((sizeof(moves)+1)*sizeof(char));
	  for(count = 0; count <= strlen(moves);count++)
	    {
	      node -> moves[count] = moves[count];
	    }
	}
    }
...
void generateAllHelper(MoveTree * root, int n_moves, const char * state, char * movelist, int ind)
{
  ...
  for(a = 0; a < 4; a++)
    {
      dup_state = malloc(sizeof(char)*(strlen(state)+1));
      for(b = 0; b <= strlen(state); b ++)
	{
	  dup_state[b]=state[b];
	}
      if(move(dup_state, m[a]))
	{
	  movelist[ind] = m[a];
	  movelist[ind+1] = '\0';
...
{ 
  char * movelist;
  MoveTree* tr;
  movelist = malloc(sizeof(char)*(n_moves + 1));
  movelist[0] = '\0';
  tr = MoveTree_create(state, movelist);
  generateAllHelper(tr, n_moves, state, movelist, 0);
  free(movelist);
  return tr;
}
...
char * solve(char * state)
{
    ...
    char *find;
    int count;
    rt = generateAll(state, MAX_SEARCH_DEPTH);
    locate = MoveTree_find(rt, FINAL_STATE);
    if(locate!=NULL)
      {
	find = malloc(sizeof(char)*(strlen(locate->moves)+1));
	for(count = 0; count <= strlen(locate->moves);count++)
	  {
	    find[count] = locate->moves[count];
	  }
      }
    else
      {
