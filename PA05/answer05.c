...
int * readInteger(char * filename, int * numInteger)
{
  ...
  while(fscanf(input, "%d", &placeholder) != EOF) //Scans the file to get the length in order to produce an adaquete array length
    {
      * numInteger = *numInteger + 1; //Index of length of array in file
    }
  array = malloc((* numInteger) * sizeof(int)); //Allocates memory for the array to be made

  fseek(input, 0 , SEEK_SET); //resets pointer of file back to index 0

  while(count2 < * numInteger) //places elements of file into new array
    {
...
char * * readString(char * filename, int * numString)
{
  ...
  char** array; //array to store the contents of the opened file
  int count1 = 0;
...
int compstring(const void *p1, const void *p2)
{
  ...
  compare=strcmp(*(char*const*)p1,*(char*const*)p2);
